...
$this->params['breadcrumbs'][] = ['label' => $model->project->title, 'url' => ['/project/view', 'id' => $model->project->project_id]];
...
<div class="task-view">
    ...
    <p class="text-center">
        <? if (Yii::$app->user->can('updateProject', ['project' => $model->project])):?>
            <?= Html::a('Update', ['task-update', 'id' => $model->task_id], ['class' => 'btn btn-primary']) ?>
        <? elseif (Yii::$app->user->can('updateTaskStatus', ['task' => $model])):?>
            <?= Html::a('Update status', ['task-update', 'id' => $model->task_id], ['class' => 'btn btn-primary']) ?>
        <? endif;?>
        ...
        <?endif;?>
        <?= Html::a('See Doers', ['doers', 'task_id' => $model->task_id], ['class' => 'btn btn-info']) ?>
    </p>
